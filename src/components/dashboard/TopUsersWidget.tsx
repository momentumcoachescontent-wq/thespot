...
const TopUsersWidget = () => {
    ...
    const [users, setUsers] = useState<{ username: string; count: number; isMe?: boolean; rank?: number }[]>([]);
    ...
    useEffect(() => {
        const fetchUserAndData = async () => {
            ...
            (data || []).forEach((d: any) => {
                ...
                if (!map[username]) map[username] = { count: 0, id: d.author_id };
                map[username].count++;
            });

            const sorted = Object.entries(map)
                .map(([username, info]) => ({ username, count: info.count, isMe: info.id === user?.id, id: info.id }))
